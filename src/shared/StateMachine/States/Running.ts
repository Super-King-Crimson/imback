...
export class Running extends MovementState {
    ...
    public override Enter(prevState: MovementState): Maybe<MovementState> {
        ...
        if (prevState !== this._airborneState)
            this.humanoid.WalkSpeed += MovementState.runBoost;

        return undefined;
    }

    public override Exit(nextState: MovementState): void {
        print("jump over, next state:");
        print(nextState);

        if (
            nextState !== this._airborneState &&
            nextState !== this._jumpingState
        ) {
            this.humanoid.WalkSpeed -= MovementState.runBoost;
        }
    }
    ...
    public override OnInputEnded(_input: InputObject): Maybe<MovementState> {
        super.OnInputEnded(_input);

        return undefined;
    }

    public override OnHeartbeat(delta: number): Maybe<MovementState> {
